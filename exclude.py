...
def update_pet_mood():
    moods = ["happy", "hungry", "confused", "zooming", "slightly evil"]
    virtual_pet["mood"] = random.choice(moods)
    virtual_pet["energy"] = max(0, virtual_pet["energy"] - random.randint(5, 20))
...
while dog:
    ...
    if user_input == "n" or user_input == "no":
        print(f"Okay, stopping now! You said you liked roblox {like_count} times.")
        dog = False  # Ends the loop if the answer is no
        continue  # Don't execute the rest after exit
    ...
    # Update virtual pet
    update_pet_mood()
    print(f"{virtual_pet['name']} is currently {virtual_pet['mood']} with {virtual_pet['energy']} energy.")

    # NEW: Inventory logic
    inventory = ["rubber chicken", "banana peel", "infinite loop"]
    found_item = random.choice(["sparkling rock", "glitched pixel", "404 hat", "mystery goo"])
    if found_item not in inventory:
        inventory.append(found_item)
        print(f"Inventory updated! You found a {found_item}")
    print("Inventory now contains:", inventory)

    # NEW: Random event
    event_roll = random.randint(1, 10)
    if event_roll == 7:
        print("EASTER EGG UNLOCKED: You found the Legendary Debug Duck!")
        inventory.append("Legendary Debug Duck")

    # Absurd chaos continues
    print("Teleporting to the fridge")
    print("Charging emotional batteries")
    print("Decrypting banana protocols")
    print("Dancing with shadows")
...
# FINAL BOSS SEQUENCE (NEW)
print("\n--- FINAL BOSS APPROACHING ---")
boss_power = random.randint(100, 700)
print("Final boss power level:", boss_power)
print("Comparing to your chaos energy:", energy_level)
time.sleep(0.5)

if energy_level > boss_power:
    print("🎉 You overwhelmed the final boss with pure nonsense energy!")
    print("The multiverse is saved... for now.")
else:
    print("💥 The final boss has absorbed your chaos!")
    print("Fallback plan: engage stealth banana.")
...
# End sequence
print("\n--- SYSTEM SHUTDOWN ---")
print("Uploading remaining nonsense to the cloud...")
print("Powering down imagination modules.")
